...
void save_measurment_start_header_to_flash(BattParameters *bat_param, uint8_t charger_state)
{
    ...
    param_tab[1] = calculate_measurment_sampling_time(bat_param);
    ...
    param_tab[5] = (uint8_t)(bat_param->batt_set_current >> 8) & 0xFF;
...
void save_measurment_end_header_to_flash(BattParameters *bat_param)
{
     ...
     {
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle1 >> 8) & 0xFF;
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle1 & 0xFF);
     }else if(bat_param->current_cycle == 2)
     {
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle2 >> 8) & 0xFF;
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle2 & 0xFF);
     }if(bat_param->current_cycle == 3)
     {
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle3 >> 8) & 0xFF;
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle3 & 0xFF);
     }if(bat_param->current_cycle == 4)
     {
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle4 >> 8) & 0xFF;
        param_tab[3] = (uint8_t)(bat_param->batt_capacitance_cycle4 & 0xFF);
     }
    ...
    for (i = 0; i < lenght; ++i)
    {
        ...
        for (j = 7; j >= 0; --j)
        {
            ...
            if (((current_param_address[i] >> j) & 0x01) == 1)
            {
                ...
                write_byte(addr + (uint32_t)i, current_param_address[i]);
...
void read_measurment_data_from_flash(BattParameters *bat_param)
{
    ...
    }else
    {
        ...
        for (i=addr+9 +1; i<=addr+100; i=i+4)
        {
        ...
        for (i=addr+9 +3; i<=addr+100; i=i+4)
        {
            read_bytes(i, &data_tab[0], 2);
            if(((data_tab[0] << 8) | data_tab[1]) == 0xFFFF)
            {
